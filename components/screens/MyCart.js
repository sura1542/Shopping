...
const MyCart = ({navigation}) => {
  ...
  const checkOut = async () => {
    ...
    navigation.navigate('Home', {totalAfterDiscount});
  };
  ...
  const handleSubtraction = () => {
    const subtractValue = parseFloat(inputValue);
    if (!isNaN(subtractValue)) {
      const newTotal = total - subtractValue;
      setTotal(newTotal >= 0 ? newTotal : 0);
      setInputValue('');
    }
  };

  const handleSubtractionPercentage = () => {
    const percentage = parseFloat(inputPercentage);
    if (!isNaN(percentage) && percentage >= 0 && percentage <= 100) {
      const subtractValue = (total * percentage) / 100;
      const newTotal = total - subtractValue;
      setTotal(newTotal >= 0 ? newTotal : 0);
      setInputPercentage('');
    }
  ...
  const handleSubtractionPoint = () => {
    const subtractValue = parseFloat(inputPoint);
    if (!isNaN(subtractValue)) {
      const newTotal = total - subtractValue;
      setTotal(newTotal >= 0 ? newTotal : 0);
      setInputPoint('');
  ...
  return (
    <View
      ...
      <View
        ...
        <TouchableOpacity
          onPress={() => (total != 0 ? checkOut(discount) : null)}
          style={{
            width: '86%',
            height: '90%',
